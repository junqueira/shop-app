...
class CustomerTests(APITestCase):
    def setUp(self):
        User.objects.create_superuser('admin', 'admin@shop.com.br', 'admin_pass')
        self.customer = Customer.objects.create(name='joao do shopping', email='jose@shop.com')
    ...
    def test_customer_create(self):
        ...
        data = {"name": "joao do shopping", "email": "jsilva@jmail.com"}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_customer_create_duplicated_email(self):
        url = reverse('customers-list')
        data = {"name": "joao do shopping", "email": "jose@shop.com"}
        response = self.client.post(url, data, format='json')
    ...
    def test_customer_update(self):
        data = {"name": "joao do shopping e Silva", "email": "jose@shop.com"}
        url = reverse('customers-detail', kwargs={'pk': self.customer.id})
